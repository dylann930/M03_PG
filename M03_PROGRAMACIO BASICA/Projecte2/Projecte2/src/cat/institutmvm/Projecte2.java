...
public class Projecte2 {
    ...
    private static final String MSG_24 = " anys";
    

    public static void main(String[] args) {
        int clientes, error = 0, error1, a = 1, consulta = 0, tipoCliente = 0, contador = 0, contador1 = 0, contador2 = 0, contador3 = 0, total = 0, cambio = 0; // Declaramos variables de tipo entero.
        ...
        for(int i = 0; i < edad.length - 1; i++){ //Con dos bucles comparamos dos posiciones seguidas de la array edad y si un valor es mas grande que el siguiente intercambiamos las posicinones.
            for(int j = i + 1; j < edad.length; j++){ //Todo esto hasta tener todas las arrays ordenadas de menor a mayor edad, realizando el cambio en las 4 arrays para no desordenar los valores.
                if(edad[i] > edad[j]){
                    cambio = id[i];
                    id[i] = id[j];
                    id[j] = cambio;
        ...
        if(consulta == 1){ //Aquí le pedimos si quiere consultar un resumen estadístico de los datos de clientes introducidos.
            ...
            System.out.println("");
            for(int i = 0; i < edad.length; i++){ //Mediante este bucle almacenamos todas las edades en la variable total.
                total = total + edad[i];
            }
            System.out.println(MSG_23 + (float)total/clientes + MSG_24); //Mostramos la media de edad dividiendo el total de edades entre la cantidad de clientes.
            System.out.println("");
